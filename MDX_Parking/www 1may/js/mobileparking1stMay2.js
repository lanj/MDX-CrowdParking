 ...
 */
	        ...
	        window.eqfeed_callback = function(results) {
	          ...
	          for (var i = 0; i < results.features.length; i++) {
	            ...
	            var marker = new google.maps.Marker({
				 ...
				 google.map.features.setProperty(name:"Access",true);
				 
				 marker.setMap(map);
				 addListenerMarker(marker, streets, bays , map );
				 //addListenerDetails(marker,street, bays , map);
                 maps.event.addDomListener( document.getElementByTag("a"), 'click', notifyFirebase);
	          }
		        ...
		        {
					var parkinContent = "<div><p><b>Street: </b> "  +
					aStreet + " </p> <p ><b>Restrictions:</b>" + 
					aBay + "  " + " (last updated: 15|04|16 ) </p>" + 
					"<p id = report  align=center > <a href = # > Report issues</a> </p>
					 <p  id=park align=center> <a href= #  data-role= button target =_self > Park </a></p>" +
					 "<p  id=leave align=center><a href=# target = _self data-role= button > Leave </a></p> 
				     </div> " ;
				  ...
				  // changed to dom listener 1st may
	 		      google.maps.event.addListener(aMarker, 'click', function(event) {
					  
						  
						  map.data.setProperty(function(feature) {
						      return {
								  var place = feature.getProperty('street');
								  if(aMarker.getPosition.equals(event.Latlng)){
									  alert("same position");
					   ...
					   $('#park').click(function(event){
								...
								aMarker.setProperty(name:"Access",false);
						
						        aMarker.toGeoJson(callback:function(geodata));
			...
			//		infodata[2] = map.data.features.Data.Features.getProperty().val;
					...
					var dataX = [{ mark:aMarker , streets: s , coordinates : infodata[0] , baytype: b  , Accessibility: null }];
						
						
					var geodata = var data = [ { name: s, Access: 'false', baytype: b, lat: 39.984, lng: -75.343 },];
